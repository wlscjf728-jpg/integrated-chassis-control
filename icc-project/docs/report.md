...
경로 추종 조건에서는 단순 yaw rate feedback만으로는 A1 DLC의 lateral deviation이 충분히 줄지 않았다. 원인은 현재 위치의 cross-track error만 보는 feedback 제어가 코너 진입에서 위상 지연을 만들기 때문이었다. 비례 gain을 올리면 처음에는 lateral deviation이 줄지만, 어느 순간부터 LTR과 sideslip이 악화되고 응답이 흔들렸다. 이 현상은 P 제어만으로 phase lag를 보상하려 할 때 나타나는 한계로 해석했다.

이를 해결하기 위해 전방 경로 곡률 preview feedforward를 넣었다.
...
| C1 single bump 또는 C2 sweep CDC 효능 | `aw_rms` 감소율 또는 peak 감소 | `ctrl_vertical.m`에 skyhook CDC를 구현하고, 횡/종가속도 기반 `c_base` scheduling으로 roll/pitch와 bump 이후 body motion을 줄이도록 설계함. 기본 `grade.m`에는 ride KPI가 출력되지 않으므로 최종 판정은 채점자가 C1/C2의 `aw_rms` 또는 suspension peak를 산출해 확인하는 방식이 필요함. | +2 후보 |
...
시간이 더 있었다면 첫째, lateral controller를 명시적인 state feedback 형태로 정리하고, $Q/R$ weight를 바꿔가며 LQR 결과와 현재 PI/ESC gain을 비교해보고 싶다. 둘째, coordinator는 현재 하중 기반 배분과 friction-circle 제한을 사용하지만, 이를 weighted least squares 형태로 더 명확히 정식화할 수 있다. 셋째, C1/C2 road input에 대해 CDC가 ride comfort KPI를 얼마나 개선하는지도 추가 검증할 수 있다.
...
- `ctrl_lateral.m`: yaw-rate PI, beta damping ESC, AYC yaw moment, 경로 곡률 preview feedforward 적용.
